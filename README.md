...
- **실기 검증 필요(미검증 상태)**: BLE 광고와 스캔을 한 어댑터에서 동시에 안정적으로 수행할 수 있는지, `bluezero.adapter.Adapter` + 원시 D-Bus `PropertiesChanged` 구독 조합이 실제로 반복 하트비트를 놓치지 않고 받아오는지, `broadcaster.Beacon`의 광고 중지 방법. 검증 전까지 `setup-door-lock.sh`에 BLE 의존성(`bluez`/`python3-dbus`/`python3-gi`/`bluezero`)을 추가하지 않는다.
